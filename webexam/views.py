...
def exam_test(instance):
    if request.method == 'GET':
        ...
        subject_data = exam.generate_subject()
        # g.subject_data=subject_data
        args_data = exam.parse_argument()
        session['args_data'] = args_data

        return redirect(url_for('exam_test', instance=instance))
        # return
        # render_template('exam_test.html',subject_data=subject_data,instance=instance,title=u'WebExam-练习')
